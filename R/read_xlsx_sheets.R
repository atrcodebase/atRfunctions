...
read_xlsx_sheets = function(path, guess_max = 5000000, convert_to_na = c("N/A", "-", " ")){
  df <- purrr::map2(path, getSheetNames(path), ~ readxl::read_excel(.x, sheet = .y, guess_max=guess_max, na=convert_to_na, .name_repair = "minimal"), .progress = TRUE) |> setNames(openxlsx::getSheetNames(path))
